...
type AuraDict = {
  ...
  categorySea: string;
  categoryBio: string;
  categoryClimate: string;
  categoryHazards: string;
  categoryPlanner: string;
  emptyCategory: string;
  categoryIndex: string;
};

type AuraEntity = Record<string, any>;
...
const _escapeHtml = (raw: any): string =>
  String(raw ?? "")
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
...
class BeregynyaAuraCard extends HTMLElement {
  ...
  private _markdownCard: any | null = null;
  private _activeCategory: AuraCategoryId | null = null;
  private _timezoneIndex = 0;
  private _clockTouchStartX: number | null = null;
  ...
  private _ensureRuntimeState(): void {
    if (!this.shadowRoot) {
      this.attachShadow({ mode: "open" });
    }
    ...
    if (this._refreshPromise === undefined) this._refreshPromise = null;
    if (this._snapshot === undefined) this._snapshot = null;
    if (this._error === undefined) this._error = null;
    if (typeof this._timezoneIndex !== "number") this._timezoneIndex = 0;
    if (this._activeCategory === undefined) this._activeCategory = null;
    if (this._clockTouchStartX === undefined) this._clockTouchStartX = null;
    if (this._markdownCard === undefined) this._markdownCard = null;
  }
  ...
  private _entities(): AuraEntity[] {
    return Array.isArray(this._snapshot?.entities) ? this._snapshot.entities : [];
  }

  private _entityMap(): Map<string, AuraEntity> {
    const map = new Map<string, AuraEntity>();
  ...
  private _entityValue(entityMap: Map<string, AuraEntity>, entityId: string): any {
    return entityMap.get(entityId)?.value;
  }

  private _entityNumeric(entityMap: Map<string, AuraEntity>, entityId: string): number | null {
    const value = this._entityValue(entityMap, entityId);
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : null;
  }

  private _numberLocale(): string {
    switch (this._lang()) {
      case "ru":
        return "ru-RU";
      case "ua":
  ...
  private _formatNumber(value: any, maximumFractionDigits = 1): string | null {
  ...
  private _formatTemp(value: any, unit = "°"): string {
    const formatted = this._formatNumber(value, 1);
    return formatted ? `${formatted}${unit}` : this._t("unavailable");
  }

  private _formatTempRange(maxRaw: any, minRaw: any): string {
  ...
  private _forecastDayLabel(dateRaw: any, fallbackIndex: number): string {
    const parsed = typeof dateRaw === "string" || typeof dateRaw === "number" ? new Date(dateRaw) : null;
    if (parsed && !Number.isNaN(parsed.getTime())) {
      return new Intl.DateTimeFormat(this._numberLocale(), { weekday: "short" })
        .format(parsed)
        .replace(/\.$/, "")
        .trim()
        .toLowerCase();
  ...
  private _forecastDayIcon(day: any): string | null {
    const gif = this._url(day?.weather_icon_gif_url);
    const img = this._url(day?.weather_icon_url) || this._url(day?.weather_icon_webp_url);
    return img || gif;
  }

  private _riskScore(value: any): number {
  ...
  private _timezones(): Array<Record<string, string>> {
    ...
    const zones = zonesRaw
      ...
      .filter((row: any) => row.timezone || row.time);
    ...
    const fallback = generated ? new Date(generated) : new Date();
    return [
      {
  ...
  private _activeTimezoneRow(): Record<string, string> {
  ...
  private _switchTimezone(direction: 1 | -1): void {
    const zones = this._timezones();
  ...
  private _categoryDefs() {
    return [
      {
        id: "sea_beach" as AuraCategoryId,
        icon: "🌊",
        title: this._t("categorySea"),
        match: (id: string) =>
          id.startsWith("sensor.sea_temperature_") ||
          id.startsWith("sensor.wave_") ||
          id.startsWith("sensor.beach_") ||
          id.startsWith("sensor.jellyfish_") ||
          id.startsWith("sensor.rip_current_") ||
          id.startsWith("sensor.tide_") ||
          id.startsWith("sensor.ocean_current_") ||
          id === "sensor.current_risk" ||
          id.startsWith("sensor.algae_"),
      },
      {
        id: "bio_allergy" as AuraCategoryId,
        icon: "🦟",
        title: this._t("categoryBio"),
        match: (id: string) =>
          id.startsWith("sensor.pollen_") ||
          id === "sensor.ambrosia_risk" ||
          id === "sensor.allergy_index" ||
          id === "sensor.asthma_risk" ||
          id.startsWith("sensor.tiger_mosquito_") ||
          id === "sensor.mosquito_index" ||
          id.startsWith("sensor.tick_") ||
          id.startsWith("sensor.bite_"),
      },
      {
        id: "climate_stress" as AuraCategoryId,
        icon: "☀️",
        title: this._t("categoryClimate"),
        match: (id: string) =>
          id.startsWith("sensor.uv_dose_") ||
          id.startsWith("sensor.astro_uv_") ||
          id === "sensor.astro_solar_elevation" ||
          id.startsWith("sensor.heat_") ||
          id === "sensor.wbgt_c" ||
          id.startsWith("sensor.dehydration_") ||
          id.startsWith("sensor.thunderstorm_") ||
          id.startsWith("sensor.air_quality_") ||
          id.startsWith("sensor.saharan_dust_") ||
          id.startsWith("sensor.smoke_"),
      },
      {
        id: "hazards_alerts" as AuraCategoryId,
        icon: "🚨",
        title: this._t("categoryHazards"),
        match: (id: string) =>
          id.startsWith("sensor.earthquake_") ||
          id.startsWith("sensor.wildfire_") ||
          id.startsWith("sensor.hazard_") ||
          id.startsWith("sensor.cap_"),
      },
      {
        id: "planner_personal" as AuraCategoryId,
        icon: "📅",
        title: this._t("categoryPlanner"),
        match: (id: string) => id.startsWith("sensor.aura_"),
      },
    ];
  }

  private _categorySummary(
    id: AuraCategoryId,
    entityMap: Map<string, AuraEntity>,
    forecast: any[],
  ): { primary: string; secondary: string; score: number } {
    if (id === "sea_beach") {
      const beachScore =
        this._entityNumeric(entityMap, "sensor.beach_comfort_index") ??
        Number(forecast[0]?.beach_score ?? NaN);
  ...
  private _buildForecastSvg(forecast: any[]): string {
    const rows = forecast
      .filter((day) => Number.isFinite(Number(day?.temp_min)) && Number.isFinite(Number(day?.temp_max)))
      .slice(0, 7);
    if (rows.length === 0) {
    ...
    const yOf = (value: number): number => top + ((max - value) / span) * (height - top - bottom);

    const maxPoints = rows
      .map((row, idx) => `${(left + idx * xStep).toFixed(1)},${yOf(Number(row.temp_max)).toFixed(1)}`)
      .join(" ");
    const minPoints = rows
      .map((row, idx) => `${(left + idx * xStep).toFixed(1)},${yOf(Number(row.temp_min)).toFixed(1)}`)
      .join(" ");
    const labels = rows
      .map((row, idx) => {
  ...
  private _getMarkdownCard(): any | null {
  ...
  private _renderDebugFallback(lines: string[]): void {
  ...
  private _bindStructuredEvents() {
    const prevBtn = this._root.querySelector<HTMLElement>("[data-tz-nav='prev']");
    const nextBtn = this._root.querySelector<HTMLElement>("[data-tz-nav='next']");
    prevBtn?.addEventListener("click", () => this._switchTimezone(-1));
    nextBtn?.addEventListener("click", () => this._switchTimezone(1));

    const clockPanel = this._root.querySelector<HTMLElement>("[data-clock-panel='1']");
    ...
    this._root.querySelectorAll<HTMLElement>("[data-category]").forEach((button) => {
      button.addEventListener("click", () => {
        const categoryId = String(button.dataset.category || "") as AuraCategoryId;
        this._activeCategory = categoryId;
        this._render();
      });
    });

    const backdrop = this._root.querySelector<HTMLElement>("[data-modal-backdrop='1']");
    const modal = this._root.querySelector<HTMLElement>("[data-modal-card='1']");
    const close = this._root.querySelector<HTMLElement>("[data-modal-close='1']");
  ...
  private _renderStructuredSkeleton() {
    this._root.innerHTML = `
  ...
  private _renderStructuredCard() {
    ...
    const weatherIcon =
      this._url(this._snapshot?.meta?.icons?.weather_current?.icon_url) ||
      this._url(this._snapshot?.meta?.icons?.weather_current?.icon_gif_url) ||
      this._forecastDayIcon(forecast[0]);
    const temp =
      this._entityNumeric(entityMap, "sensor.apparent_temperature") ??
      this._entityNumeric(entityMap, "sensor.weather_temperature") ??
      Number(forecast[0]?.temp_max ?? NaN);
    ...
    const weatherSummary =
      weatherSummaryRaw === null || weatherSummaryRaw === undefined || weatherSummaryRaw === "unavailable"
        ? this._t("weather")
        : String(weatherSummaryRaw);
    ...
    const weatherForecast = forecast
      ...
      .map((day, index) => {
        ...
        return `<div class="weather-day">
          ...
          ${
            dayIcon
              ? `<img class="weather-day-icon" src="${dayIcon}" alt="${_escapeHtml(dayLabel)}"/>`
              : `<div class="weather-day-icon placeholder">⛅</div>`
          }
        ...
        const body =
          rows.length > 0
            ? rows
                ...
                .join("")
            : `<div class="modal-empty">${_escapeHtml(this._t("emptyCategory"))}</div>`;
...
if (!window.customCards.find((it: any) => it.type === _auraCard.type)) {
